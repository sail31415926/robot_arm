...
@brief  ArmPoseCommand 订阅执行器（无 GUI）
@version 1.0
@date   2026-06-04

订阅 /robot_arm/pose_command，收到后执行对应预设运动：
  STOWED   → 所有关节归零（关节空间直接下发，不经过 IK）
  OBSERVE  → POSE_OBSERVE（下方修改）
  SHOOTING → 消息中的 target_pose_x/y/z + SHOOTING_ORI 朝向（下方修改）
             pan_deg / tilt_deg 同步记录日志（供云台控制器使用）
...
# ── SHOOTING（拍摄位朝向） ────────────────────────────────────────────────────
# 拍摄位的 XYZ 由 ArmPoseCommand 消息携带，此处仅配置末端朝向。
SHOOTING_ORI = dict(
    roll_deg =  90.0,   # 末端横滚角（°）
    pitch_deg=  10.0,   # 末端俯仰角（°），正值向下俯视
    yaw_deg  =   0.0,   # 末端偏航角（°）
)
...
POSE_CMD_TOPIC = '/robot_arm/pose_command'  # 订阅话题
...
class PoseCommandExecutor(Node):
    ...
    def __init__(self):
        ...
        self.create_subscription(ArmPoseCommand, POSE_CMD_TOPIC, self._on_cmd, 10)

        self._planning  = False
        self._stop_req  = False

        self.get_logger().info(
            f'PoseCommandExecutor 就绪，订阅 {POSE_CMD_TOPIC}')
        self.get_logger().info(
            '  STOWED  → 所有关节归零（关节空间直接下发，不经过 IK）')
        self.get_logger().info(
            ...
            f'{POSE_OBSERVE["z"]:.2f})')
        self.get_logger().info(
            f'  SHOOTING → XYZ from msg + ori=('
            f'{SHOOTING_ORI["roll_deg"]:.1f}°, {SHOOTING_ORI["pitch_deg"]:.1f}°, '
            f'{SHOOTING_ORI["yaw_deg"]:.1f}°)')

    # ── 订阅回调 ──────────────────────────────────────────────────────────────
    def _on_cmd(self, msg: ArmPoseCommand):
        state = msg.target_pose_state
        speed = msg.transition_speed

        state_name = STATE_NAMES.get(state, str(state))
        speed_name = SPEED_NAMES.get(speed, str(speed))
        log = f'收到 ArmPoseCommand: {state_name}  速度={speed_name}'
        if state == ArmPoseCommand.POSE_STATE_SHOOTING:
            log += (f'  目标=({msg.target_pose_x:.3f},'
                    f'{msg.target_pose_y:.3f},{msg.target_pose_z:.3f})'
                    f'  pan={msg.pan_deg:.1f}°  tilt={msg.tilt_deg:.1f}°')
        self.get_logger().info(log)

        if self._planning:
            self.get_logger().warn('规划中，忽略新指令（先等当前运动完成或发 STOP）')
            return

        if state == ArmPoseCommand.POSE_STATE_STOWED:
            self._stop_req = False
            self._go_stowed(speed)
            return
        elif state == ArmPoseCommand.POSE_STATE_OBSERVE:
            p = POSE_OBSERVE
            target = (p['x'], p['y'], p['z'],
                      p['roll_deg'], p['pitch_deg'], p['yaw_deg'])
        elif state == ArmPoseCommand.POSE_STATE_SHOOTING:
            ori = SHOOTING_ORI
            target = (msg.target_pose_x, msg.target_pose_y, msg.target_pose_z,
                      ori['roll_deg'], ori['pitch_deg'], ori['yaw_deg'])
        else:
            self.get_logger().error(f'未知 target_pose_state={state}')
            return

        sp = SPEED_PARAMS.get(speed, SPEED_PARAMS[ArmPoseCommand.SPEED_NORMAL])
        self._stop_req = False
        threading.Thread(
            target=self._plan_ptp,
            args=(*target,
                  sp['v_pos'], sp['a_pos'], sp['j_pos'],
                  sp['v_ori'], sp['a_ori'], sp['j_ori']),
            daemon=True).start()

    # ── STOWED：关节空间归零，不经过 IK ──────────────────────────────────────
    def _go_stowed(self, speed: int):
    ...
    def _solve_and_send(self, all_pts, desc):
        ...
        for idx, (t_pt, wx, wy, wz, qx, qy, qz, qw) in enumerate(all_pts):
            if self._stop_req:
                self.get_logger().info('■ 规划中止'); return
            ...
            if sol is None:
                ...
                if joint_pos:
                    ...
                    self.get_logger().error(
                        ...
                        f'pos=({wx:.3f},{wy:.3f},{wz:.3f})'); return
    ...
    def _plan_ptp(self, x, y, z, roll_deg, pitch_deg, yaw_deg,
        ...
        try:
            ...
            if start is None or q_cur is None:
                self.get_logger().error('✗ TF 未就绪'); return
            ...
            while True:
                ...
                if res == Result.Error:
                    self.get_logger().error('✗ Ruckig 求解失败'); return
