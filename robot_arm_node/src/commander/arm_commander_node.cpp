 ...
 * @brief ArmCommanderNode 实现 —— 状态机 + 3 Action Server + 4 Service +
 * ArmStatus 广播
 *
 * 状态机 IDLE/MOVING/REACHED/STOPPED/ERROR 的转换、Action goal
 * 接受/执行/终态判定、 急停与清错、驱动层 Trigger
 * 同步转发（仿真不可用则跳过）均在此。Action 执行体 （mtp/em/track_execute）在
 * handle_accepted 派生线程里跑，调用对应 server 的 execute。
 *
 * @version 1.0
 * @date 2026-07-01
...
/**
 * @brief 构造 Commander 节点：加载调参 → 建子系统 → 注册全部 Action/服务/话题。
 *
 * 构造顺序有硬性要求：tuning::declare_and_load 必须在任何子系统构造**之前**
 * 调用。各 server 在自己的执行路径上通过 tuning::params() 取容差/档位/超时，
 * 这里晚一步它们就会拿到结构体默认值而不是 YAML 里的值（且不报错，静默失效）。
 */
ArmCommanderNode::ArmCommanderNode() : rclcpp::Node("arm_commander") {
    using rclcpp_action::CancelResponse;
    using rclcpp_action::GoalResponse;

    // ⚠️ 必须在任何子系统构造之前：容差/档位/超时等由各 server
    // 在自己的执行路径上
    //    通过 tuning::params() 读取，这里晚一步它们就会拿到结构体默认值而非
    //    YAML 值。
    ...
    monitor_ = std::make_unique<ExecutionMonitor>(
        get_logger(), is_stopped_fn, [this]() { motion_->stop(); });
    mtp_srv_ = std::make_unique<MoveToPoseServer>(
        *this, *motion_, *status_, *monitor_,
        [this]() { return get_observe_pose(); });
    mtj_srv_ = std::make_unique<MoveToJointServer>(*this, *motion_, *status_,
                                                   *monitor_);
    em_srv_ = std::make_unique<TrajectoryShotServer>(*this, *motion_, *status_,
                                                     *monitor_, is_stopped_fn);
    track_srv_ =
        std::make_unique<TrackTargetServer>(*this, *status_, is_stopped_fn);
    // 速度流控制：topic 流式，不占状态机。与 action 的互斥由「控制模式」保证 ——
    // 速度流只在 JOINT_VELOCITY 模式下下发，而轨迹类动作执行前会切回
    // TRAJECTORY， 一切模式速度流自己就停了（两者都发同一个 JTC
    // 话题，靠模式串行化，不会打架）。
    vstream_srv_ =
        std::make_unique<VelocityStreamServer>(*this, *status_, is_stopped_fn);

    // ── Action Server（全部 ACCEPT_AND_EXECUTE，非空闲检查在执行线程内
    // abort，与 Python 一致）──
    ...
    mtp_server_ = rclcpp_action::create_server<MoveToPose>(
        ...
        [this](
            std::shared_ptr<rclcpp_action::ServerGoalHandle<MoveToPose>> gh) {
    ...
    mtj_server_ = rclcpp_action::create_server<MoveToJoint>(
        ...
        [this](
            std::shared_ptr<rclcpp_action::ServerGoalHandle<MoveToJoint>> gh) {
    ...
    em_server_ = rclcpp_action::create_server<TrajectoryShot>(
        ...
        [](const rclcpp_action::GoalUUID&,
           std::shared_ptr<const TrajectoryShot::Goal>) {
            return GoalResponse::ACCEPT_AND_EXECUTE;
        },
        [this](
            std::shared_ptr<rclcpp_action::ServerGoalHandle<TrajectoryShot>>) {
    ...
    track_server_ = rclcpp_action::create_server<TrackTarget>(
        ...
        [](const rclcpp_action::GoalUUID&,
           std::shared_ptr<const TrackTarget::Goal>) {
            return GoalResponse::ACCEPT_AND_EXECUTE;
        },
        ...
        [this](
            std::shared_ptr<rclcpp_action::ServerGoalHandle<TrackTarget>> gh) {
    ...
    status_timer_ =
        create_wall_timer(100ms, [this]() { publish_status(); }, cb_group_);
    ...
    stop_srv_ = create_service<robot_arm_interfaces::srv::ArmStop>(
        SERVICE_ARM_STOP,
        std::bind(&ArmCommanderNode::on_arm_stop, this, _1, _2),
        rmw_qos_profile_services_default, cb_group_);
    enable_srv_ = create_service<robot_arm_interfaces::srv::ArmEnable>(
        SERVICE_ARM_ENABLE,
        std::bind(&ArmCommanderNode::on_arm_enable, this, _1, _2),
        rmw_qos_profile_services_default, cb_group_);
    homing_srv_ = create_service<robot_arm_interfaces::srv::ArmHoming>(
        SERVICE_ARM_HOMING,
        std::bind(&ArmCommanderNode::on_arm_homing, this, _1, _2),
        rmw_qos_profile_services_default, cb_group_);
    reset_error_srv_ = create_service<robot_arm_interfaces::srv::ArmResetError>(
        SERVICE_ARM_RESET_ERROR,
        std::bind(&ArmCommanderNode::on_arm_reset_error, this, _1, _2),
        rmw_qos_profile_services_default, cb_group_);
    ...
    // 轨迹类动作在速度模式下会静默失效，执行前自动切回 TRAJECTORY（仍走
    // mode_manager 唯一入口）
    mode_switch_cli_ = create_client<SwitchControlMode>(
        SWITCH_MODE_SRV, rmw_qos_profile_services_default, cb_group_);

    RCLCPP_INFO(get_logger(),
                "Arm Commander 已就绪  |  状态=%s  |  action: %s / %s / %s / "
                "%s  |  速度流: %s",
                state_name(state()), ACTION_MOVE_TO_POSE, ACTION_MOVE_TO_JOINT,
                ACTION_TRAJECTORY_SHOT, ACTION_TRACK_TARGET,
                TOPIC_FOLLOW_COMMAND);
}

// ── 状态机
// ──────────────────────────────────────────────────────────────────────
/**
 * @brief 状态枚举转可读字符串（日志用）。
 *
 * @param s Commander 状态。
 * @return 状态名字符串。
 */
const char* ArmCommanderNode::state_name(CommanderState s) {
    switch (s) {
        case CommanderState::IDLE:
            return "IDLE";
        case CommanderState::MOVING:
            return "MOVING";
        case CommanderState::REACHED:
            return "REACHED";
...
/**
 * @brief 切换 Commander 状态，同步更新 moving 标志并打日志。
 *
...
void ArmCommanderNode::transition(CommanderState new_state) {
    ...
    RCLCPP_INFO(get_logger(), "状态: %s → %s", state_name(old),
                state_name(new_state));
}

/**
 * @brief 判断是否处于可接受新动作的空闲态。
 *
 * REACHED 也算空闲 —— 它表示上一个动作已正常完成，不该阻塞下一个动作。
 *
 * @return 空闲（IDLE 或 REACHED）返回 true。
 */
...
void ArmCommanderNode::mtp_execute(
    std::shared_ptr<rclcpp_action::ServerGoalHandle<MoveToPose>> gh) {
    if (!is_idle()) {
        RCLCPP_WARN(get_logger(), "拒绝 goal: 当前状态=%s，非空闲",
                    state_name(state()));
    ...
    try {
        auto result =
            std::make_shared<MoveToPose::Result>(mtp_srv_->execute(gh));
        if (is_stopped()) {
            RCLCPP_INFO(
                get_logger(),
                "MoveToPose 执行期间被急停，goal 终止，状态保持 STOPPED");
...
// ───────────────────────────────────────────────────── 与 mtp_execute
// 同构，两点不同：
//   ① 不动 current_pose_state ——
//   关节空间点到点是示教/标定用途，不代表产品语义上的
//      「收纳/观察/拍摄」姿态；硬套一个会让 Director
//      误判。需要新语义时再加枚举。
//   ② 三类前置校验失败（out_of_range / collision / invalid_goal）视同「拒绝该
//   goal」：
//      回 IDLE 而不是进 ERROR（与 MoveToPose 的 unreachable
//      一致，因为没下发任何指令， 设备本身没故障）。
void ArmCommanderNode::mtj_execute(
    std::shared_ptr<rclcpp_action::ServerGoalHandle<MoveToJoint>> gh) {
    if (!is_idle()) {
        RCLCPP_WARN(get_logger(), "拒绝 goal: 当前状态=%s，非空闲",
                    state_name(state()));
    ...
    try {
        auto result =
            std::make_shared<MoveToJoint::Result>(mtj_srv_->execute(gh));
        if (is_stopped()) {
            RCLCPP_INFO(
                get_logger(),
                "MoveToJoint 执行期间被急停，goal 终止，状态保持 STOPPED");
            status_->set_command_state(cmd_id, ArmStatus::RESULT_ABORTED);
            gh->abort(result);
            return;
        }
...
// ──────────────────────────────────────────────────
void ArmCommanderNode::em_execute(
    std::shared_ptr<rclcpp_action::ServerGoalHandle<TrajectoryShot>> gh) {
    if (!is_idle()) {
        RCLCPP_WARN(get_logger(), "拒绝 goal: 当前状态=%s，非空闲",
                    state_name(state()));
    ...
    try {
        auto result =
            std::make_shared<TrajectoryShot::Result>(em_srv_->execute(gh));
        if (is_stopped()) {
            RCLCPP_INFO(
                get_logger(),
                "TrajectoryShot 执行期间被急停，goal 终止，状态保持 STOPPED");
            status_->set_command_state(cmd_id, ArmStatus::RESULT_ABORTED);
            gh->abort(result);
            status_->set_camera_ready(false);
            return;
        ...
        if (result->success) {
            ...
            // 拒绝本次 goal 恢复空闲，不进 ERROR（否则需要 reset_error
            // 才能继续）
            RCLCPP_WARN(get_logger(),
                        "目标不可达（IK 无解），拒绝本次 goal，恢复空闲");
            transition(CommanderState::IDLE);
...
// ── ArmTrackTarget 执行线程（server 自行终结
// goal，本层只做状态转换）────────────
void ArmCommanderNode::track_execute(
    std::shared_ptr<rclcpp_action::ServerGoalHandle<TrackTarget>> gh) {
    ...
    try {
        ...
        if (is_stopped()) {
            RCLCPP_INFO(get_logger(),
                        "TrackTarget 执行期间被急停，状态保持 STOPPED");
            status_->set_command_state(cmd_id, ArmStatus::RESULT_ABORTED);
            return;
        }
...
void ArmCommanderNode::on_arm_stop(
    ...
    if (state() == CommanderState::MOVING) {
        ...
        response->success = true;
        response->message =
            "已急停速度流，状态 → STOPPED（需 ArmResetError 复位）";
    } else {
        // 没有 goal 在跑也没在发速度流：状态机不动，但速度总线仍已闩锁 ——
        // Director 可能正在直发关节速度，那条路不经状态机
        response->success = true;
        response->message =
            std::string("当前状态=") + state_name(state()) +
...
// ── 轨迹类动作的模式前置条件
// ──────────────────────────────────────────────────────
//   MoveToPose / MoveToJoint / TrajectoryShot 全都靠 JTC 执行；速度模式下 JTC
//   被停用，
//   轨迹发出去也不会动（表现为动作静默超时）。这类动作的语义本身就要求轨迹模式，
//   所以这里**自动切回 TRAJECTORY** 而不是让上层先手动切一次 ——
//   与「速度流不自动切模式」
//   的约定不冲突：速度流是持续控制权，动作是一次性位置指令。
//   切换仍然只经
//   mode_manager（唯一入口），失败则放弃执行并让调用方拿到明确原因。
/**
 * @brief 确保当前处于 TRAJECTORY 模式，必要时经 mode_manager 自动切回。
 *
 * MoveToPose / MoveToJoint / TrajectoryShot 全靠 JTC 执行，速度模式下 JTC 被
 * 停用，轨迹发出去也不动（表现为动作静默超时）。这类动作的语义本身就要求轨迹
 * 模式，所以这里自动切 —— 与「速度流不自动切模式」的约定不冲突：速度流是持续
 * 控制权，动作是一次性位置指令。切换仍只经 mode_manager 这个唯一入口。
 *
 * @param why 出参，失败原因（可为 nullptr）。
 * @return 已在或已成功切到 TRAJECTORY 返回 true。
 */
bool ArmCommanderNode::ensure_trajectory_mode(std::string* why) {
    ...
    if (!resp || !resp->success) {
        if (why)
            *why = std::string("切回 TRAJECTORY 失败：") +
                   (resp ? resp->message : "无应答");
        RCLCPP_ERROR(get_logger(), "%s", why ? why->c_str() : "");
        return false;
    }
...
/**
 * @brief 通知 mode_manager 挂起/释放速度总线的急停闩锁。
 *
 * 用异步发送而不等应答：急停必须立刻返回，不能卡在等 mode_manager 上。
 * 服务不存在时（如 MuJoCo 后端）静默跳过 —— 那类后端也没有速度控制器。
 *
 * @param engage true 挂起闩锁（禁止速度指令），false 释放。
 */
void ArmCommanderNode::latch_velocity_estop(bool engage) {
    if (!vel_estop_cli_->service_is_ready()) {
        RCLCPP_DEBUG(get_logger(), "%s 不可用，跳过速度总线闩锁",
                     VELOCITY_ESTOP_SRV);
        return;
    }
...
void ArmCommanderNode::on_arm_enable(
    const std::shared_ptr<robot_arm_interfaces::srv::ArmEnable::Request>
        request,
    ...
    response->success = ok;
    response->message =
        std::string("伺服") + (request->enable ? "使能" : "下电") + ": " + msg;
...
void ArmCommanderNode::on_arm_homing(
    ...
    if (!is_idle()) {
        ...
        response->message = std::string("拒绝回零：当前状态=") +
                            state_name(state()) + "，非空闲";
        RCLCPP_WARN(get_logger(), "ArmHoming: %s", response->message.c_str());
        return;
    }
    transition(CommanderState::MOVING);
    motion_->go_to_joints(tuning::params().homing_joints,
                          tuning::params().homing_duration_sec);
...
void ArmCommanderNode::on_arm_reset_error(
    ...
    std::shared_ptr<robot_arm_interfaces::srv::ArmResetError::Response>
        response) {
    auto [drv_ok, drv_msg] =
        call_driver_trigger(drv_recover_cli_, ARM_NODE_RECOVER_SRV);
    latch_velocity_estop(false);  // 解除速度总线急停闩锁（与 ArmStop 成对）

    const auto current = state();
    if (current == CommanderState::ERROR ||
        current == CommanderState::STOPPED) {
        ...
        response->message = std::string("已复位 ") + state_name(current) +
                            "→IDLE，驱动层: " + drv_msg;
    } else {
        response->success = drv_ok;
        response->cleared_error_code = 0;
        response->message = std::string("状态=") + state_name(current) +
                            "，驱动层recover: " + drv_msg;
    }
...
// ── OBSERVE 预定义位姿（从 ROS param
// 读）────────────────────────────────────────
/**
 * @brief 从 ROS 参数读取 OBSERVE 预定义位姿。
 *
 * 每次调用都重读参数，故运行时改参数即刻生效（无需重启节点）。
 *
 * @return OBSERVE 位姿（位置 m，姿态角为度）。
 */
ArmCommanderNode::ArmPose ArmCommanderNode::get_observe_pose() {
    ...
    p.pitch =
        static_cast<float>(get_parameter("pose_observe_pitch").as_double());
